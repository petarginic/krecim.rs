...
const serviceLinks = ["Krečenje", "Gletovanje", "Gipsani Radovi", "Adaptacija", "Lepljenje Tapeta"];
const companyLinks = [
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div className="md:col-span-1">
            <div className="flex items-center gap-1">
              <span className="text-2xl font-bold text-white">krecim</span>
              <span className="text-2xl font-bold text-purple-400">.rs</span>
            </div>
            <p className="mt-3 text-sm leading-relaxed">
              Profesionalno krečenje, gletovanje i gipsani radovi u Beogradu.
          ...
          {/* Usluge */}
          <div>
            <h4 className="text-white font-semibold text-sm mb-4">Usluge</h4>
            <ul className="space-y-2.5">
              {serviceLinks.map((link) => (
                <li key={link}>
                  <a href="/#usluge" className="text-sm hover:text-white transition-colors duration-200">
                    {link}
                  </a>
                </li>
              ))}
          ...
          {/* Kompanija */}
          <div>
            <h4 className="text-white font-semibold text-sm mb-4">Kompanija</h4>
            <ul className="space-y-2.5">
            ...
            <ul className="space-y-2.5">
              {blogPosts.slice(0, 5).map((post) => (
                <li key={post.slug}>
                  <Link
                    href={`/blog/${post.slug}`}
                    className="text-sm hover:text-white transition-colors duration-200 line-clamp-1"
                  >
                    {post.title}
                  </Link>
                </li>
